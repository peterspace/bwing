...
const ChainsGnosis = require('../../models/Chains/Gnosis.js');

//Require tokenId
//==========================================={chain}======================================
//======================================={Create Token}=============================================================================================
const addNewChainsGnosis = async (req, res) => {
  const {
    chainId,
    name,
    address,
...
const duplicate = await ChainsGnosis.findOne({ address }).collation({ locale: 'en', strength: 2 }).lean().exec()

if (duplicate) {
    return res.status(409).json({ message: 'Duplicate Toke address' })
}

let token = new ChainsGnosis();
...
// To get all networks
const getChainsGnosis = async (req, res) => {
  const chain = await ChainsGnosis.find();
  if (!chain?.length) {
...
//   const token = await ChainsGnosis.findById(id).exec()
...
const updateChainsGnosis = async (req, res) => {
  const {
    id,
    chainId,
    name,
...
const deleteChainsGnosis = async (req, res) => {
  const { id } = req.body;
