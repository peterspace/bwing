...
const ChainsEthereum = require('../../models/Chains/Ethereum.js');

//Require tokenId
//==========================================={chain}======================================
//======================================={Create Token}=============================================================================================
const addNewChainsEthereum = async (req, res) => {
  const {
    chainId,
    name,
    address,
...
const duplicate = await ChainsEthereum.findOne({ address }).collation({ locale: 'en', strength: 2 }).lean().exec()

if (duplicate) {
    return res.status(409).json({ message: 'Duplicate Toke address' })
}

let token = new ChainsEthereum();
...
// To get all networks
const getChainsEthereum = async (req, res) => {
  const chain = await ChainsEthereum.find();
  if (!chain?.length) {
...
//   const token = await ChainsEthereum.findById(id).exec()
...
const updateChainsEthereum = async (req, res) => {
  const {
    id,
    chainId,
    name,
...
const deleteChainsEthereum = async (req, res) => {
  const { id } = req.body;
