...
const ChainsFantom = require('../../models/Chains/Fantom.js');
//Require tokenId
//==========================================={chain}======================================
//======================================={Create Token}=============================================================================================
const addNewChainsFantom = async (req, res) => {
  const {
    chainId,
    name,
    address,
...
const duplicate = await ChainsFantom.findOne({ address }).collation({ locale: 'en', strength: 2 }).lean().exec()

if (duplicate) {
    return res.status(409).json({ message: 'Duplicate Toke address' })
}

let token = new ChainsFantom();
...
// To get all networks
const getChainsFantom = async (req, res) => {
  const chain = await ChainsFantom.find();
  if (!chain?.length) {
...
//   const token = await ChainsFantom.findById(id).exec()
...
const updateChainsFantom = async (req, res) => {
  const {
    id,
    chainId,
    name,
...
const deleteChainsFantom = async (req, res) => {
  const { id } = req.body;
