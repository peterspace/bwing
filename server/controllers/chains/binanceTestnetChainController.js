...
const ChainsBinanceTestnet = require('../../models/Chains/BinanceTestnet.js');

//Require tokenId
//==========================================={chain}======================================
//======================================={Create Token}=============================================================================================
const addNewChainsBinanceTestnet = async (req, res) => {
  const {
    chainId,
    name,
    address,
...
const duplicate = await ChainsBinanceTestnet.findOne({ address }).collation({ locale: 'en', strength: 2 }).lean().exec()

if (duplicate) {
    return res.status(409).json({ message: 'Duplicate Toke address' })
}

let token = new ChainsBinanceTestnet();
...
// To get all networks
const getChainsBinanceTestnet = async (req, res) => {
  const chain = await ChainsBinanceTestnet.find();
  if (!chain?.length) {
...
//   const token = await ChainsBinanceTestnet.findById(id).exec()
...
const updateChainsBinanceTestnet = async (req, res) => {
  const {
    id,
    chainId,
    name,
...
const deleteChainsBinanceTestnet = async (req, res) => {
  const { id } = req.body;
