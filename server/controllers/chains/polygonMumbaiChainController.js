const ChainsPolygonMumbai = require('../../models/Chains/PolygonMumbai.js');

//Require tokenId
//==========================================={chain}======================================
//======================================={Create Token}=============================================================================================
const addNewChainsPolygonMumbai = async (req, res) => {
  const {
    chainId,
    name,
    address,
...
const duplicate = await ChainsPolygonMumbai.findOne({ address }).collation({ locale: 'en', strength: 2 }).lean().exec()

if (duplicate) {
    return res.status(409).json({ message: 'Duplicate Toke address' })
}

let token = new ChainsPolygonMumbai();
...
// To get all networks
const getChainsPolygonMumbai = async (req, res) => {
  const chain = await ChainsPolygonMumbai.find();
  if (!chain?.length) {
...
//   const token = await ChainsPolygonMumbai.findById(id).exec()
...
const updateChainsPolygonMumbai = async (req, res) => {
  const {
    id,
    chainId,
    name,
...
const deleteChainsPolygonMumbai = async (req, res) => {
  const { id } = req.body;
