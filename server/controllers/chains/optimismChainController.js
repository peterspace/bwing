...
const ChainsOptimism = require('../../models/Chains/Optimism.js');

//Require tokenId
//==========================================={chain}======================================
//======================================={Create Token}=============================================================================================
const addNewChainsOptimism = async (req, res) => {
  const {
    chainId,
    name,
    address,
...
const duplicate = await ChainsOptimism.findOne({ address }).collation({ locale: 'en', strength: 2 }).lean().exec()

if (duplicate) {
    return res.status(409).json({ message: 'Duplicate Toke address' })
}

let token = new ChainsOptimism();
...
// To get all networks
const getChainsOptimism = async (req, res) => {
  const chain = await ChainsOptimism.find();
  if (!chain?.length) {
...
const getOneChain = async (req, res) => {
  ...
  const token = await ChainsOptimism.findById(id).exec()
...
const updateChainsOptimism = async (req, res) => {
  const {
    id,
    chainId,
    name,
...
const deleteChainsOptimism = async (req, res) => {
  const { id } = req.body;
