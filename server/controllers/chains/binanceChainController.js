 ...
 const ChainsBinance  = require('../../models/Chains/Binance.js');


//Require tokenId
//==========================================={chain}======================================
//======================================={Create Token}=============================================================================================
const addNewChainsBinance = async (req, res) => {
  const {
    chainId,
    name,
    address,
...
const duplicate = await ChainsBinance.findOne({ address }).collation({ locale: 'en', strength: 2 }).lean().exec()

if (duplicate) {
    return res.status(409).json({ message: 'Duplicate Toke address' })
}

let token = new ChainsBinance();
...
// To get all networks
const getChainsBinance = async (req, res) => {
  const chain = await ChainsBinance.find();
  if (!chain?.length) {
...
//   const token = await ChainsBinance.findById(id).exec()
...
const updateChainsBinance = async (req, res) => {
  const {
    id,
    chainId,
    name,
...
const deleteChainsBinance = async (req, res) => {
  const { id } = req.body;
