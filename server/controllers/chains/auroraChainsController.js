...
const ChainsAurora = require("../../models/Chains/Aurora");

//Require tokenId
//==========================================={chain}======================================
//======================================={Create Token}=============================================================================================
const addNewChainsAurora = async (req, res) => {
  const {
    chainId,
    name,
    address,
...
const duplicate = await ChainsAurora.findOne({ address }).collation({ locale: 'en', strength: 2 }).lean().exec()

if (duplicate) {
    return res.status(409).json({ message: 'Duplicate Toke address' })
}

let token = new ChainsAurora();
...
// To get all networks
const getChainsAurora = async (req, res) => {
  const chain = await ChainsAurora.find();
  if (!chain?.length) {
...
//   const token = await ChainsAurora.findById(id).exec()
...
const updateChainsAurora = async (req, res) => {
  const {
    id,
    chainId,
    name,
...
const deleteChainsAurora = async (req, res) => {
  const { id } = req.body;
