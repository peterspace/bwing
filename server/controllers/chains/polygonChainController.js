...
const ChainsPolygon = require('../../models/Chains/Polygon.js');

//Require tokenId
//==========================================={chain}======================================
//======================================={Create Token}=============================================================================================
const addNewChainsPolygon = async (req, res) => {
  const {
    chainId,
    name,
    address,
...
const duplicate = await ChainsPolygon.findOne({ address }).collation({ locale: 'en', strength: 2 }).lean().exec()

if (duplicate) {
    return res.status(409).json({ message: 'Duplicate Toke address' })
}

let token = new ChainsPolygon();
...
// To get all networks
const getChainsPolygon = async (req, res) => {
  const chain = await ChainsPolygon.find();
  if (!chain?.length) {
...
const getOneChain = async (req, res) => {
  ...
  const token = await ChainsPolygon.findById(id).exec()
...
const updateChainsPolygon = async (req, res) => {
  const {
    id,
    chainId,
    name,
...
const deleteChainsPolygon = async (req, res) => {
  const { id } = req.body;
