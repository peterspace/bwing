...
const ChainsGoerliEth = require('../../models/Chains/GoerliEth.js');
//Require tokenId
//==========================================={chain}======================================
//======================================={Create Token}=============================================================================================
const addNewChainsGoerliEth = async (req, res) => {
  const {
    chainId,
    name,
    address,
...
const duplicate = await ChainsGoerliEth.findOne({ address }).collation({ locale: 'en', strength: 2 }).lean().exec()

if (duplicate) {
    return res.status(409).json({ message: 'Duplicate Toke address' })
}

let token = new ChainsGoerliEth();
...
// To get all networks
const getChainsGoerliEth = async (req, res) => {
  const chain = await ChainsGoerliEth.find();
  if (!chain?.length) {
...
const getOneChain = async (req, res) => {
  ...
  const token = await ChainsGoerliEth.findById(id).exec()
...
const updateChainsGoerliEth = async (req, res) => {
  const {
    id,
    chainId,
    name,
...
const deleteChainsGoerliEth = async (req, res) => {
  const { id } = req.body;
