...
const ChainsAvalanche = require('../../models/Chains/Avalanche.js');

//Require tokenId
//==========================================={chain}======================================
//======================================={Create Token}=============================================================================================
const addNewChainsAvalanche = async (req, res) => {
  const {
    chainId,
    name,
    address,
...
const duplicate = await ChainsAvalanche.findOne({ address }).collation({ locale: 'en', strength: 2 }).lean().exec()

if (duplicate) {
    return res.status(409).json({ message: 'Duplicate Toke address' })
}

let token = new ChainsAvalanche();
...
// To get all networks
const getChainsAvalanche = async (req, res) => {
  const chain = await ChainsAvalanche.find();
  if (!chain?.length) {
...
const getOneChain = async (req, res) => {
  ...
  const token = await ChainsAvalanche.findById(id).exec()
...
const updateChainsAvalanche = async (req, res) => {
  const {
    id,
    chainId,
    name,
...
const deleteChainsAvalanche = async (req, res) => {
  const { id } = req.body;
