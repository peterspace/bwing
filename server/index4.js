...
io.on('connection', (socket) => {
  ...
  socket.on('setup', (userData) => {
    ...
    console.log({ userRoom1: userData._id });
  ...
  socket.on('new message', async (newMessageRecieved) => {
    var content = newMessageRecieved.content;
    console.log({ newMessageRecieved });
    if (!content) return console.log('chat empty');

    const usersList = newMessageRecieved?.users;
    ...
    if (usersList.length > 0) {
      usersList.map((id) => {
        ...
        console.log({ id });
        console.log({ userRoom2: id });

        // if (user._id == newMessageRecieved.sender._id) return;

        // socket.in(id).emit('message recieved', newMessageRecieved); // send to the whole group
        // socket.to(id).emit('message recieved', newMessageRecieved); // send to the whole group
        io.to(id).emit('message recieved', newMessageRecieved); // send to the whole group

      });
    } else {
      console.log('from the bottom');

      socket.emit('message recieved', newMessageRecieved); // send to the sender only
    }
  ...
  //   //   const clientUser = newMessageRecieved?.user;
  //   //   // const adminUser = newMessageRecieved?.admin;
  ...
  //   //   //  const users = [clientUser, adminUser];
