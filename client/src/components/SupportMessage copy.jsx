...
const SupportMessage = (props) => {
  const { allMessages, page, fetchAllMessages } = props;
  const { user } = useSelector((state) => state.user);
  const messageEndRef = useRef(null); // for message autoscroll to the most recent message
  ...
  useEffect(() => {
    socket.on('message recieved', (newMessageRecieved) => {
      if (
        ...
        // }
        console.log('no current chat, await notification');
      } else {
